...
#if defined(NDEBUG)
# error "Junipercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe1dad3e9f37c3fe0ca2bbb2a395ed4149eeef6f03be924aa926fc08a0a5706c5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Junipercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Junipercoin Signed Message:\n";
...
{
    // Junipercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 84000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 84000); // Junipercoin: 84k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = .35 * 24 * 60 * 60; // Junipercoin: .35 days
static const int64 nTargetSpacing = .25 * 60; // Junipercoin: .25 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Junipercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Junipercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[3] = 0xdu;
        hashGenesisBlock = uint256("0xe6bdae02897bb20b5f6c62103e8d08878462dfc511802afdae6edf1d69853b4d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 8/9/2022 - JumboJupiter pirates TopGun : Maverick";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a9a8717d40ccd1c99f6106c9a91177062d3b1365b4d6c954fb714ac37f4c64b4b0d887676b25abdbae9c2492f63fc4101f5ae835c2c7394ef7e38161955aa0c2") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1660072019;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084605669;

        if (fTestNet)
        {
            block.nTime    = 1660072005;
            block.nNonce   = 386263437;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x87d568c45e828e8e2a101b168cc4985b070ae753636d31fc1356e945bcf2cff7"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc6, 0xb1, 0xdu }; // Junipercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("JunipercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("JunipercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static JunipercoinMiner(CWallet *pwallet)
{
    printf("JunipercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("junipercoin-miner");
    ...
    try { loop {
        ...
        printf("Running JunipercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("JunipercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&JunipercoinMiner, pwallet));
}
